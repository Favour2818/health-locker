...
/**
* Encrypt multiple values (batch) — used for multi-field records
*/
export async function createEncryptedBatchInput(contractAddress, userAddress, valuesArray) {
const fhe = getFheInstance();
if (!fhe) throw new Error("FHE instance not initialized. Call initializeFheInstance() first.");

console.log(`🔐 Batch encrypting values ${valuesArray} for user: ${userAddress}`);
...
// Store handle and proof just like createEncryptedInput
inputHandles.push(result.handles[0]);
proofs.push(result.inputProof);
}

return {
encryptedData: inputHandles, // array of bytes32 handles
proof: proofs, // array of proofs (matching the contract input)
};
}
